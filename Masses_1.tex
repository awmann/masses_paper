...
Extracting separations and position angle measurements followed a similar multi-step procedure across all instruments excluding NRM data. Our method is based largely on that described in \citet{2016ApJ...817...80D} and \citet{Dupuy2017}, which is built on the techniques from \citet{2008ApJ...689..436L} and \citet{2010ApJ...721.1725D}. Users can consult these papers for more details, but we describe most of the process here. 
...
Resulting fits all have $\chi^2_\nu$ values, ranging from 0.1 to 2, within the expected distribution given the number measurements per orbit. This was in part a consequence of our method of assigning errors to literature data (Section~\ref{sec:litas}). However, we achieved reasonable $\chi^2_\nu$ values even when all or nearly all the data were from our own measurements, indicating that our astrometry measurements have reasonable assigned errors.
...
Another possibility for this scatter is stellar physics not well accounted for in Equation~\ref{eqn:mmk2}, e.g., metallicity. We investigate this in the next Section.
