...
\title{Weighing the Stars I:\\ the mass-luminosity relation from 0.70$M_\odot$ to the hydrogen burning limit
...
\author{Adam Kraus}%[0000-0001-9811-568X]
...
\author{Raquel Martinez}%[0000-0002-5127-8606]
\affiliation{Department of Astronomy, The University of Texas at Austin, Austin, TX 78712, USA}
...
\item Check no even power fit?
\end{itemize}


\section{Introduction}\label{sec:intro}
Over the last decade, M dwarfs have become critically important across a wide range of astrophysics. On the small scale, M dwarfs have become attractive targets for the identification and characterization of exoplanets. The small size, low mass, and low luminosity of late-type dwarfs facilitates the discovery of small \citep[e.g.][]{Muirhead2012,Martinez:2017aa,Mann:2018}, young \citep[e.g.,][]{2012ApJ...753..142B,Mann2016b}, and habitable-zone \citep[e.g.,][]{Tarter2007,Shields:2016aa,2017Natur.544..333D} exoplanets. Close-in, rocky planets are also significantly more common around M dwarfs than their Sun-like counterparts \citep{Dressing2013,2013PNAS..11019273P,Mulders2015,Gaidos2016b}, and are more likely to harbor close-in multi-planet systems \citep[e.g.,][]{Muirhead2015,Ballard2016}. 
...
We analyzed a mix of AO data from our own program with NIRC2 on Keck with images from the archives of Keck, the Canada France Hawaii Telescope (CFHT), the Very Large Telescope, and Gemini observatory. We separate the observations and reduction by instrument/telescope below. 
I
\subsubsection{Keck II/NIRC2 Imaging and Masking}

Between March 2002 and November 2017, 48 of our targets were observed a total of 166 times, all using the facility imager, NIRC2, on Keck II atop Maunakea. The majority of this data ($>100$ observations) were taken as part of our long-term monitoring program spanning 2015A to 2016B. The rest were downloaded from the Keck Observatory Archive (KOA).

For our observations, we always used the vertical angle mode, both for imaging and non-redundant aperture masking (NRM). We selected the smallest pixel scale ($9.952 \pm 0.002$ mas/pix). Depending on brightness and conditions, imaging was usually taken with either the $K'$ or $K_{\rm{cont}}$ filters, and NRM was always taken using the 9-hole mask and $K'$ filter. After acquiring the target and allowing the AO loops to close, we took four to ten images, adjusting coadds and integration time based on the brightness of the target, and dithering to at least two locations on the chip. For the NRM observations, we took 6-8 interferograms, again adjusting the integration time as appropriate, but always using a single coadd. As most of our targets are bright, observations were generally taking using the Natural Guide Star (NGS) system \citep{2000PASP..112..315W}, only utilizing the Laser Guide Star (LGS) mode for the faintest ($R\gtrsim13$) targets or in mixed conditions.

NIRC2 data taken from the archive covered a wide range of observing modes, filters, and cameras, although the majority was taken with the narrow camera and one of the $H$- or $K$-band filters. We included nearly all data with a clear detection of both binary components independent of the observing setup. We discarded saturated images, those that taken with the coronagraph atop one or both components, and those with extremely poor AO correction. 

The same basic data reduction was applied to both observations from our own program and those from the archive, following the procedure from \citet{Kraus2016a}. To briefly summarize, we linearized each frame then dark and flat corrected using calibration data taken the same night. In cases where no appropriate darks or flats were taken in the same night, we use a set from the nearest available night. We interpolated over ``dead'' and ``hot'' pixels, which were identified from superflats and superdarks built from data spanning 2006 to 2016. Because flats are rarely taken in narrowband filters, we used superflats built from the nearest (in wavelength) broadband filter where appropriate. Pixels with flux levels $>10\sigma$ above the median of the eight adjacent pixels (primarily cosmic rays) were replaced with the median. Images were visually inspected as part of identifying the binary location, and a handful ($\ll$1\%) of images were negatively impacted by our cosmic ray removal (e.g., removal of part of the source). These images were manually corrected.

Reduction of masking observations follows the appendix of \citet{Kraus2008} with the aid of the `Sydney' aperture-masking interferometry code\footnote{\href{https://github.com/mikeireland/idlnrm}{https://github.com/mikeireland/idlnrm}}. To remove systematics, the observation of each target observed with NRM was paired with a calibration observation of another nearby star taken in the same night with a similar setup and observational parameters. Binary system profiles were then fit to the closure phase to produce detection limits and uncertainties. More details on the reduction of masking data can be found in \citet{2006ApJ...650L.131L}, \citet{Kraus2008} and \citet{2012ApJ...744..120E}.


\subsubsection{CFHT/PUEO Imaging}

In total, 31 of our targets were observed with the 3.6m Canada-France-Hawaii Telescope (CFHT) using the Adaptive Optics Bonnette \citep[AOB, often referred to as PUEO after the Hawaiian owl,][]{1994SPIE.2201..833A} with the KIR infrared camera \citep{1998SPIE.3354..760D}. After removing bad observations (e.g., saturated, unresolved, or poor AO correction), 187 unique epochs were used, all of which we downloaded from the CFHT archive at the Canadian Astronomy Data Centre \footnote{\href{http://www.cadc-ccda.hia-iha.nrc-cnrc.gc.ca/}{CADC archive}}. Observations spanned December 1997 to January 2007, covering most of the time PUEO was in use at CFHT (1996 to 2011). Most observations were taken in a few year period from 2000 to 2003. 

We applied flat-fielding and dark correction using a set of superflats and superdarks built by splitting the datasets into 6 month blocks and combining calibration data within the block. We identified bad pixels by making large median stacks of science data taken across dozens of objects, which were then replaced with the median of nearby pixels. To identify cosmic rays, we median stacked consecutive images of each target (at a fixed location), then compared the stack to individual images. Data points $>8\sigma$ above the robust mean of the stacks are replaced with the median of the eight surrounding points. Because PUEO/KIR data were taken in sets of $>$5 images before the object was dithered, this median stacking was effective for removing nearly all cosmic rays.


\subsubsection{VLT/NaCo Imaging}

We downloaded AO-assisted images from the ESO archive taken with the Nasmyth Adaptive Optics System Near-Infrared Imager and Spectrograph (NAOS-CONICA, or NaCo) instrument on VLT\footnote{\href{http://archive.eso.org/eso/eso_archive_main.html}{ESO Archive}}. Data spanned November 2001 to November 2017, with about half the epochs taken from 2001 to 2005. Data include 42 unique epochs over 19 of our targets, excluding saturated or otherwise unusable data. Observations were taken covering a wide range of filters, cameras, and observing patterns, but the majority was taken using either the S13 camera with the broadband $K_s$ and $L$, or narrowband [FeII] and [Br$\gamma$] filters, and using a standard 2-4 point dither pattern. 

We applied flat-fielding and dark corrections to each observation using the standard set of calibrations taken each night as part of the VLT queue. In the case where a calibration was missing or unusable, we used the nearest (in time) set of calibration data matching the filter (for flats) and exposure setup (for darks) as appropriate. Flats taken in broadband filters were used for narrowband flat-fielding when there were insufficient narrowband flats available. We built a bad pixel mask using median stacks of hundreds of consecutive images after applying flat and dark corrections, accounting for significant variations in the mask over the 16 years of data. For cosmic ray removal, we used the L.A. Cosmic software \citep{2001PASP..113.1420V}.


\subsubsection{Gemini/NIRI Imaging}

We retrieved 14 epochs for 7 of our targets from the Gemini archive, all taken with the AO imager NIRI. All observations were taken between August 2008 and February 2011. Most observations were taken with the f32 camera, but using wide range of filters and observational procedures.

We applied flat and dark corrections to each set of images using the standard calibration data taken as part of the Gemini queue, usually within 24h of the target observations. In most cases, flats taken in broadband filters were used for narrowband flat-fielding. Bad pixels were identified from image stacks, and observations with the target near or on top of a heavily impacted pixel were discarded. We used the L.A. Cosmic software \citep{2001PASP..113.1420V} for the identification and removal of cosmic rays. 

\subsubsection{Position angles, separations, and contrast ratios}\label{sec:astrometry}

Extracting separations and position angle measurements followed a similar multi-step procedure across all instruments excluding NRM data (which is described above). Our method is based largely on that described in \citet{2016ApJ...817...80D} and \citet{Dupuy2017}, which is built on the techniques from \citet{2008ApJ...689..436L} and \citet{2010ApJ...721.1725D}. Users can consult these papers for more details, but we describe most of the process below. 
...
\caption{Example results of our orbit fitting procedure for Gl301AB (left), Gl469AB (middle), and GJ3454AB (right). These highlight the range of quality seen in our orbit sample. Red points are individual measurements of the separation and position angle. Black solid line shows the best-fit (highest likelihood). Grey lines are drawn by randomly sampling 100 orbit fits from the MCMC fit to give an estimate of the errors. The dotted line marks the periastron passage, with an arrow for the orbital direction, and the dashed line notes the line of nodes. For Gl301AB, there are no data for a large section of the orbit, but the final fit is relatively well constrained. For Gl469AB, high-quality astrometry is available for the full orbit, and the resulting errors on orbital parameters are so small that the grey lines cannot be seen. All other orbits are shown in Appendix~\ref{sec:a1}}
\label{fig:orbits}
\end{center}
\end{figure*}

Resulting fits all have $\chi^2_\nu$ values, ranging from 0.1 to 2, are are all within the expected distribution given the number measurements per orbit. This was in part a consequence of our method of assigning errors to literature data (Section~\ref{sec:litas}). However, we achieved reasonable $\chi^2_\nu$ values even when all or nearly all the data were from our own measurements, indicating that our astrometry measurements have reasonable assigned errors.

For the nine systems we drew orbit posteriors from \citet{Dupuy2017}. These were fit in a nearly identical way and were provided in a similar format to those analyzed in this paper. These nine systems are labeled separately in Table~\ref{tab:sample})
...
Errors on the dynamical mass are correlated to the errors on \mks. The system mass scales with $a^3$, which depends linearly with the parallax. As a result, the parallax was a major, if not the largest, source of uncertainty on the mass for most systems. Similarly, our $K_S$ magnitudes had relatively small errors (0.01-0.03\,mag), so \mks\ errors tended to be dominated by the parallax. Because this correlation is usually along (parallel to) the direction of the \mmk\ relation, it can improve the overall fit if taken into account. Our strategy was to treat distances of each system as a free parameter in the fit, having them evolve under a strong prior from the observed parallax. The MCMC was provided $a\arcsec^3/P^2$ and $K_S$ (with uncertainties) for each system, from which total mass and \mks\ were recomputed at each step in the MCMC before making a comparison. For all systems, the output posterior on the parallax was consistent with the input prior (Figure~\ref{fig:parallax}).
...
\caption{A comparison of example input 1$\sigma$ priors (black lines) to the output posteriors (red distributions). Gl 310 represents one of the largest differences between the prior and posterior, highlighting that the measured parallax dominates the evolution. }
\label{fig:parallax}
\end{center}
\end{figure*}
...
\caption{Posterior on semi-major axis as a function of orbital period (bottom left) for the example system LHS 6167. Grey regions show 1, 2, and 3$\sigma$ (from darkest to lightest) of the points. The histograms above and to the right show the one-dimensional distributions of each parameter. The parameter fed into the \mmk\ fit is $a^3/P^2$ (in arcsec$^3$ per year$^2$), which we show in the top right inset. }
\label{fig:correlated}
\end{center}
\end{figure}

We know that for un-evolved, main-sequence stars, more massive stars should always be brighter. So we applied an additional constraint that the relation must have a negative derivative (higher \mks\ always gives lower mass) over the full range of input objects considered. We tested running without this constraint, and found similar results over most of the parameter range considered. The major difference was near the edges of the relation. With this constraint the fit would sometimes become double valued (two \mks\ values that yield the same mass) at or just beyond the input sample.
...
\enddata
\label{tab:err}
\tablecomments{This table assumes that you know \mks\ (and for the bottom values [Fe/H]) perfectly. Total errors on $M_*$ should take into account errors in the measured parameters and the relation. }
...
The values in Table~\ref{tab:err} can be used as an approximate reference for estimating errors on the output mass for a given \mks\ if combined with errors from the distance and $K_S$ magnitude. However, this can be misleading, as the posterior is not Gaussian or symmetric over much of the mass range. It is better to sample the posterior at a given \mks\ to produce a posterior in mass. To facilitate use of our \mmk\ and help provide realistic uncertainties, we include the fit posteriors and provide a simple code that provides output $M_*$ posteriors\footnote{\href{https://github.com/awmann/M_-M_K-}{https://github.com/awmann/M\_-M\_K-}} given a \mks\ and error. 

The fit with the highest likelihood yields a $\chi^2_\nu$ of 1.5 with 57 degrees of freedom (accounting for correlated errors). The larger than expected scatter is primarily driven by a handful of systems that agree with the best-fit to within $\simeq$3\%, but also have small total mass uncertainties ($\lesssim$1\%). The MCMC explores the range of solutions, so if we factor in the scatter in the final \mmk\ then the $\chi^2_\nu$ value decreases to 0.98. As a result, using the fit posteriors provides uncertainties on $M_*$ that encompass the larger scatter. Although this does not help explain its origin.

The larger than expected scatter could be due to systematically underestimated uncertainties, most likely in the input distances. As we noted with the {\it Gaia} DR2 parallaxes (Section~\ref{sec:plx}), orbital motion can shift the centroid and hence impact the measured parallax. This is particularly true for systems with orbital periods near an alias of one year (the parallax signal's period). About 1/2 of the systems included were not impacted by this because they either had have their parallaxes drawn from a nearby bound (and single) star, calculated accounting for the presence of the binary, or have orbits far from the timescale of the parallactic signal. But a small impact on the remaining half could explain the higher scatter. 

If the parallax or orbital errors were underestimated this could bias our result towards the most precise points. We explored this possibility by redoing our MCMC analysis adding a free parameter that uniformly increases the parallax uncertainty of all systems. This parameter generally made no significant difference to the least-well constrained systems, but significantly increased uncertainties on the most precisely constrained systems. The final fit parameters and resulting relation were negligibly different from those derived without the added error term, and the only meaningful change was a reduction in the $\chi^2_\nu$ of the best-fit solution. Because the change was small, we did not explore adding more sophisticated error terms to the adopted parallax, such as using the orbital fits to model the deviation in the unresolved binary centroid and hence measure the expected parallax deviation. With a larger set of targets that have parallaxes assuming a single-star as well as those accounting for binarity it will be possible to explore this issue in more detail.

Another possibility for this scatter is stellar physics not well accounted for in Equation~\ref{eqn:mmk2}, most likely metallicity. We investigate this in the next Section.

\subsection{The role of metallicity}\label{sec:metal}

We explored the expected effects of [Fe/H] on the \mmk\ relation using the Mesa Isochrones and Stellar Tracks \citep[MIST,][]{MIST0,MIST1}. We also tested the Dartmouth Stellar Evolution Database \citep[DSEP,][]{Dotter2008}, and found results to be broadly in agreement. We prefer MIST because their model atmospheres include a newer set of molecular lines (e.g., TiO) that are important in M dwarfs and may impact the synthetic photometry. The Lyon models \citep[BHAC15,][]{BHAC15} use a wrapper atmosphere in reasonable agreement with nearby stars with interferometrically measured radii \citep{Boyajian2012,Mann2013c}, and go to lower masses than DSEP and MIST. However, the BHAC15 models cover only solar metallicity. 

We show the expected \mks\ tracks from MIST for different metallicities in Figure~\ref{fig:mk_metal} alongside our empirical determinations. MIST models do not currently go below 0.1$M_\odot$, so we restricted our comparison to above that mass. For this comparison we assumed a fixed age of 5\,Gyr, although the choice of age from 1-10\,Gyr makes a negligible difference for the mass range shown. Note that metal-rich stars are expected to be {\it less luminous} in \mks\ (and $L_{\rm{bol}}$) for a fixed $M_*$, whereas the opposite trend is seen for a fixed \teff\ and hence most color selections. 
...
Above 0.4$M_\odot$ there is a slight trend for metal-rich stars in our sample to land below the median sequence, as expected from the models. However, many metal-poor stars also land below the sequence, and there is no obvious trend below this mass. We made this comparison more quantitative by interpolating over the model grid to create a predicted mass for each system using \mks\ and [Fe/H]. The predicted system masses were statistically worse than those from our best-fit relation ($\chi^2_\nu$ of 6.5 using the model grid versus 1.5 with Equation~\ref{eqn:mmk2}), and worse than simply assuming every star has [Fe/H]=-0.3 or [Fe/H]=0.0 ($\chi^2_\nu$ of 3.9 and 4.1). Allowing a zero-point correction to all metallicities (i.e., assuming they are systematically off from the model scale) improves the result ($\chi^2_\nu=4.0$), but a metallicity-free solution is still preferred. This suggests that the models are overestimating the effect of [Fe/H] on the \mmk\ relation.

The residuals from our best-fit relation confirmed a weak (or no) effect on the derived $M_*$ due to changes in [Fe/H], as we show in Figure~\ref{fig:metal_resid}. A Spearman rank test yielded no significant correlation between the residuals and [Fe/H]. We tried resampling the measurements using their uncertainties, and $<$1\% of samples showed a significant correlation. Even when we restricted the sample to the best-characterized systems ($<5\%$ precision on mass), there was still no obvious trend with [Fe/H]. 

\begin{figure}[htp]
\begin{center}
\includegraphics[width=0.47\textwidth]{residual_metal5.eps}
\caption{Fractional difference between the orbital and predicted system mass as a function of metallicity of the system. The top panel contains all systems, while the bottom shows just those with $<$5\% uncertainties on $M_*$. Points are color-coded by the masses of components, with the inner dot corresponding to the primary star's estimated mass, and the outer circle and error bars corresponding to the  companion's estimated mass. Quoted errors in mass account for the scatter in the \mmk\ relation.} 
...
To compare to the models, we fit the MIST grid points in the same manner as the empirical dataset following Equation~\ref{eqn:mmk3}. In Figure~\ref{fig:f} we show the posterior on $f$ from the model grid compared to that from the dynamical masses. While the posteriors have some overlap, more than 96\% of the difference between the two posteriors ($f_{\rm{model}}-f_{\rm{dynamical}}$) is $>0$, suggesting the models likely overpredict the effect of [Fe/H] on the \mmk\ relation. We interpret this with caution, as the result is not significant, and our current sample of binaries covers a relative narrow range of [Fe/H]. It is also unclear if this applies to the full mass range considered; there is some indication that the metallicity effect is stronger for higher-mass stars in the sample. A larger set of metal-poor ([Fe/H]$<0.5$) binaries or [Fe/H] values of higher precision will be needed to better constrain $f$ and see if this discrepancy is statistically significant (and over what masses). 
...
\citet{Delfosse2000} is one of the most commonly used \mmk\ relations, covering $0.1M_\odot<M_*0.6M_\odot$. Like our work, the calibration was build primarily on astrometric binaries. Nearly all the targets in \citet{Delfosse2000} were included in our sample, with the except of triple systems and eclipsing binaries, both of which we intentionally avoided because of the complexity of computing total masses in the first scenario and estimating \mks\ in the latter. Because of the sample overlap, consistency is expected. However, a comparison can be useful to see how past use of \citet{Delfosse2000} may change with the updated relation. 
...
Like our work, the \citet{Benedict2016} relation was also based primarily on masses derived from M dwarf astrometric binaries. The \citet{Benedict2016} uses {\it HST} fine guidance sensors, which provide absolute astrometry. Combined with radial velocity observations, this yields individual masses, while our analysis is only based on total system masses. So although our sample is larger and contains most of the targets in \citet{Benedict2016}, their analysis has several key advantages and the final fitting methodology differs. Differences between our final relations could therefore shed light on differences in the non-overlapping sample or the need to obtain individual masses. 

We compare our \mmk\ relation \citet{Benedict2016}'s in Figure~\ref{fig:benedict}. The two relations are in excellent agreement for $0.09M_\odot\lesssim M_* \lesssim 0.25M_\odot$. Below this regime, the \citet{Benedict2016} fit is anchored by the one star, GJ1245C. They have only two other stars in this low-mass regime (GJ 2005B and C) and they are comparatively poorly characterized. GJ1245AC is in our sample, but we use a parallax from \citet{GaiaDr2} for GJ 1245B, which places it $5.5\sigma$ more distant that the parallax adopted by \citet{Benedict2016}, and more consistent with our overall relation. Our parallax-free orbital parameters for this system are in excellent agreement with \citet{Benedict2016}. Since the \citet{Benedict2016} parallax did account for the binarity of this system, the origin of this difference is unclear. If GJ 1245B is itself a binary, this could explain the discrepant parallax, in which case we need to explain why GJ 1245C is significantly more luminous than similar-mass stars \citep[e.g., youth,][]{2015ApJ...800...95L,2017ApJ...834...85N}.
...
Inspection of the literature data incorporated by \citet{Benedict2016} makes the origin of this difference clear; many of these are eclipsing binaries and have $\Delta K$-band magnitudes of mixed quality and/or lack parallaxes needed to derive a precise \mks. GU Boo, for example, has absolute magnitudes estimated from an optical eclipse depth combined with bolometric corrections \citep{Lopez2005}, which are drawn from models that perform poorly on M dwarfs \citep{1998A&AS..130...65L,Hauschildt1999}. Similarly, for GJ 2069 AC (Cu Cnc) \citet{Benedict2016} adopted \mks\ from \citet{Ribas2003} that disagree with the 2MASS $K_S$ and {\it Gaia} DR2 parallax (for either AC or B) using any $\Delta K_S$. We conclude that these $M_*$ and \mks\ determinations need to be revised before including them in future analyses of the \mmk\ relation. 
%% problematic points:
...
\subsection{Summary}
The mass-luminosity has proven to be a critical tool for estimating masses of cool stars for decades, and has applications from studying extrasolar planets to measuring the initial mass function and mass-to-light ratio in galaxies. The \mmk\ relation has been particularly useful because \mks\ is easily measured if the stellar distance is known, and because it mitigates the effect of metallicity compared to other bands. With the arrival of {\it Gaia} parallaxes combined with existing 2MASS photometry, nearly all the early M-dwarfs out to $\simeq$100\,pc and late M-dwarfs out to $\simeq$50\,pc will have precise ($\lesssim1\%$) parallaxes and \mks\ magnitudes, only increasing the utility of this relation. 

We endeavored to improve on the existing \mmk\ relations and explore the reliability of model predictions by expanding the sample of calibrators and exploring the role of [Fe/H]. As part of this effort, we have been monitoring a set of nearby late K and M dwarf visual binaries using adaptive optics imaging. We combined these data with similar data from Keck, CFHT, Gemini, and VLT archives, as well as astrometric measurements from the literature. The literature astrometry was particularly important to provide the baseline on long-period systems. We used these data and parallaxes from the literature or derived from MEarth astronomy to measure the total masses of a set of 52 binaries. We combined this with nine ultracool binaries from \citep{Dupuy2017}, which were analyzed in a nearly identical way to the procedure used in this paper and provide significantly improved coverage near the hydrogen burning limit. 

Following the formalism in Section~\ref{sec:relation}, we used our dynamical total masses and resolved \mks\ magnitudes to fit for the empirical \mmk\ relation. The resulting relation provides masses precise to 2-3\% from $0.3-0.6M_\odot$ and $<5$\% over most of the M dwarf sequence. The relation also covers the entire M dwarf sequence, from $\simeq0.7M\odot$ down to the hydrogen burning limit ($\simeq0.075M_\odot$). The scatter around the best-fit is larger than expected from the mass uncertainties, especially when considering that errors in \mks\ and $M_*$ are correlated. This might be due to underestimated errors in the parallaxes due to centroid motion of the binary, or intrinsic scatter from additional physics (e.g., metallicity and age). 

Using empirically calibrated spectroscopic abundances, we explored the role of [Fe/H] on the \mmk\ relation, comparing our results to expectations from evolutionary models as a guide. While the MIST models follow our empirical relation relatively well, there is some evidence (96\% chance) that the models are overestimating the effect of [Fe/H]. We consider this suggestive, but not conclusive at this time, as the narrow metallicity range of our sample limits our ability to explore this further. 
...
As a test of this method, we tried refitting a subset of 10 randomly selected binaries twice, first doubling the error term we added, then halving it. With the exception of one system that failed to converge with the doubled input errors, overall parameters and errors did not change significantly (although the final $\chi^2_\nu$ values changed). The main reason for this is that our measurements (in particular those from NIRC2) are generally far more more precise  (with the exception of measurements from {\it HST}) and disproportionately weigh on the final solution. Further, in the case of significantly underestimated errors, the MCMC usually explores a range of solutions to result in a similarly broad posterior as the one derived using larger errors. The problem with this case is that MCMC will underweight more precise points (e.g., those from NIRC2/NaCo). Overall, many issues that might arise from a single source of data were mitigated because we used a wide range of data sources, each of which were subject to different systematics. 


The presence of unresolved tight companions could bias the overall relation. For the longest-period systems, a close-in (orbital period of $<$10 days) third star could be stable, and would not have been detected in our AO data. An unseen companion would drive the total mass of the system higher, but has much smaller effect on the total luminosity. So unresolved triples will sit {\it low} in Figure~\ref{fig:relation} (higher mass for a fixed \mks). A few stars do sit low, but no more so than expected given their mass uncertainties. The closest systems have radial velocity data, which rules out the presence of all companions massive enough to impact our results. Such triples are relatively rare \citet{Fischer1992,Raghavan2010}, especially those with a companion low-mass enough to avoid perturbing the orbits (which would be detected as a high $\chi^2_\nu$ value in our fits). We consider it unlikely that higher-order systems are significantly impacting our results.  
...
\item Our relation is only valid for main-sequence stars. Based on our selection criteria and lack of activity in the late K and early M dwarfs, we expect $>90\%$ of our sample to be older than 1-2\,Gyr \citep{2017ApJ...834...85N}. Many of the coolest stars in our sample are rapidly rotating and/or show H$\alpha$ emission \citep{Gaidos2014,2015ApJ...800...95L}, but not at atypical levels for even old stars in this mass range \citep{2015AJ....149..158S}. Based on the Lyon models \citep{BHAC15}, we advise restricting use to $>$100\,Myr above $0.4M_\odot$, $>$300\,Myr to $0.2M_\odot$, $>$500\,Myr to $0.1M_\odot$, and $>1$\,Gyr below 0.1$M_\odot$. A safer cut would be to only use this on stars $>$1 Gyr. 

\item The result is only tested over $-0.60<\rm{[Fe/H]}<+0.45$. We provide a fit that attempts to take into account changes due to [Fe/H], but the effect is poorly constrained (fractional change of $0.06\pm0.06$ in $M_*$ per dex in [Fe/H]). Assuming the target is within the metallicity range of M dwarfs in the Solar neighborhood \citep[$-0.05\pm0.18$,][]{Gaidos2014} then the uncertainty introduced by the unknown [Fe/H] value is $\simeq$1\%. However, when targeting more metal-poor populations like the thick disk or halo ($[Fe/H]\lesssim-0.5$) or  the effect on the derived $M_*$ will be $>$5\%. We advise avoiding this relation for stars known to have [Fe/H]$<-0.5$. 
...
We intentionally tried to avoid young stars when selecting the sample considered here. Although the ages of these stars is not know, preventing any study into the effects of age on the \mmk. However, our larger sample of binaries contains a range of known members of nearby young moving groups (YMGs) and the Hyades cluster. Many of these have complete or nearly complete orbital solutions, and will soon provide a powerful set of mass benchmarks with known ages. These systems also span ages from 10-650\,Myr, offering the chance to both test pre-main-sequence models of M dwarfs \citep{2015ApJ...813L..11M,2016ApJ...817..164R,2016ApJ...818..156C,2016AJ....152..175N} and explore the role of activity on M dwarf parameters \citep[e.g.,][]{Spada2013,2018arXiv180404133K}. Analysis of the effects of activity on the relation can also utilize this sample if combined with optical activity indicators and rotation periods from the Transiting Exoplanet Survey Satellite \citep[{\it TESS},][]{2014SPIE.9143E..20R}. 
...
\tablewidth{\linewidth}
\tablecaption{Binary Sample}
\tablehead{
    \colhead{Name} & \colhead{Comp} & \colhead{R.A.} & \colhead{Dec} & \colhead{$K_S$} 
 & \colhead{$\Delta K_S$} & \colhead{System $M_*$} & \colhead{[Fe/H]$^a$} & \colhead{Parallax}  & \colhead{Ref} \\
...
\tablewidth{\linewidth}
\tablecaption{Orbital Parameters}
\tablehead{
...
Here we show diagnostic plots of each of the binaries analyzed in this paper. The three binaries shown in Figure~\ref{fig:orbits} are not repeated here. 
