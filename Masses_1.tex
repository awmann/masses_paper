...
We analyzed a mix of AO data from our own program with NIRC2/Keck, and images from the archives of Keck, the Canada France Hawaii Telescope (CFHT), the Very Large Telescope, and Gemini observatory. In general we analyzed all usable (e.g., non saturated, components resolved) regardless of observing mode and filter. For our analysis, we considered a single `epoch' a set of images with a unique filter, target, or date. 
