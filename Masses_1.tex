...
\begin{abstract}
The mass-luminosity relation for late-type stars has long been a critical tool for estimating masses of late-type stars, with implications for a wide range of astrophysics. However, this is growing need both for higher-precision relations and a better understanding of systematics (e.g., metallicity) in the relation. Here we present an empirical \mmk\ relation spanning 0.70$M_\odot$ to the hydrogen burning limit ($\simeq0.075M_\odot$). The relation is derived from observations of 59 nearby binaries, whose orbits we determine using a a combination of our own monitoring with NIRC2 on Keck, archival adaptive optics data, and astrometry from the literature. From their orbital parameters, we determine the total mass of each system to better than 1\% in the best cases (median precision on $M_{\rm tot}$ of 5.6\%). We constrain the relation between \mks\ and $M_*$ by joining our mass determinations with resolved $K$-band magnitudes for each component and distance estimates for each system within a Monte Carlo Markov Chain framework. The resulting posteriors can be used to determine masses of single stars to 2-3\% over the whole mass range considered. The primary precision limit is scatter around the best-fit relation that is larger than expected by the input $M_*$ uncertainties. This may be due to intrinsic variation in the \mmk\ relation or underestimated uncertainties in the input parallaxes and/or orbital parameters. The effect of [Fe/H] on the \mmk\ relation is small (at fixed \mks\ one dex change in [Fe/H] shifts the inferred $M_*$ by 3.8$\pm$2.1\%), and likely to be negligible for metallicities in the Solar neighborhood. A sample of binaries with a wider range of abundances will be required to see if metallicity becomes important for more extreme populations (e.g., in the Galactic Halo or thick disk). The impact of [Fe/H] derived from our dynamical masses is significantly smaller than that predicted by the MESA isochrones ($4.0\sigma$), but in reasonable agreement with predictions from the Dartmouth Stellar Evolution Database. Although both model grids overpredict $M_*$ for a given \mks\ below $0.4M_\odot$.
\end{abstract}
...
\end{eqnarray}
where $b_j$ is defined in Equation~\ref{eqn:mmk2_b}. This form assumes that a linear change in [Fe/H] corresponds to a fractional change in $M_*$ (e.g., f=0.1 would correspond to a 10\% change in derived $M_*$ per dex change in [Fe/H] at a fixed \mks). This is generally consistent with the models over the metallicity range considered here (it is predicted to become increasing non-linear for [FeH]$<-0.5$). Equation~\ref{eqn:mmk3} also assumes a single $f$ over the whole mass range considered. While this is consistent with the predictions of MIST models, DSEP models show a tightening. However, there are too few points in our sample (and too narrow a range of metallicities) to explore $f = \mathscr{F}($\mks$)$.
...
There is some evidence in the binary sample that [Fe/H] effects are stronger at the high-mass end (see Figure~\ref{fig:relation}), consistent with DSEP prediction. We explored this by repeating the analysis excluding binaries with one or more component masses below $0.35M_\odot$. While we found a marginally larger $f$ value using just the higher-mass sample ($f=0.056\pm0.035$\,dex$^{-1}$), while the MIST-based $f$ in the restricted mass range is smaller ($f=0.12\pm0.02$\,dex$^{-1}$). This yields a discrepancy of 1.6$\sigma$, suggesting the problem is primarily driven by stars with $M_*<0.35M_\odot$. 
...
%% GJ 2005: They report a different mass in the text then in the table... ?? which is correct?
