...
Many of our targets do not have entries in the second data release of {\it Gaia} \citep[DR2, ][]{GaiaDr2,2018arXiv180409365G}. They were most likely excluded because centroid shifts due to orbital motion prevented a single-star solution (a requirement to be included in DR2). We also found significant differences between the {\it Gaia} values and earlier measurements \citep[including from {\it Gaia} DR1, ][]{gaiadr1}. Many wide triples in {\it Gaia} DR2 (where the third star is several arc seconds away, and hence easily resolved) have significantly different parallaxes reported for the unresolved pair than for the wider companion. Orbital motion is likely impacting the final parallax values or uncertainties. The problem should be fixed in future Gaia data releases when there is sufficient data to fit for binarity parameters. We found no such issues with our other parallax sources. 
...
For GJ 2005BC and Gl 900BC the 2MASS PSF includes flux from the A component. In both cases we use our AO data to measure $\Delta K$ between all three components. The total $K_S$ magnitude given in Table~\ref{tab:sample} is for BC. 
...
Our sample encompasses almost an order of magnitude in mass and hence a range of underlying stellar physics. No single $\alpha$ value is expected to fit over the full sequence. Instead, we assumed that $\alpha$ depends on \mks, which we approximated as a polynomial of the form:
