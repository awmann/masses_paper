...
To compare to the models, we fit the MIST grid points in the same manner as the empirical dataset following Equation~\ref{eqn:mmk3}. In Figure~\ref{fig:f} we show the posterior on $f$ from the model grid compared to that from the dynamical masses. While the posteriors have some overlap, more than 96\% of the difference between the two posteriors ($f_{\rm{model}}-f_{\rm{dynamical}}$) is $>0$, suggesting the models likely overpredict the effect of [Fe/H] on the \mmk\ relation. We interpret this with caution, as the result is currently not significant. It is also unclear if this applies to the full mass range considered; there is some indication that the metallicity effect is stronger for higher-mass stars in the sample. A larger set of metal-poor ([Fe/H]$<0.5$) binaries or [Fe/H] values of higher precision will be needed to better constrain $f$ and see over what mass range, if any, this discrepancy is statistically significant. 
