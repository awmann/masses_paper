...
%\author[0000-0002-6879-3639]{Jonathan Irwin}
%\affiliation{Harvard-Smithsonian Center for Astrophysics, 60 Garden St., Cambridge, MA, 02138}
...
\begin{abstract}
The mass-luminosity relation for late-type stars has long been a critical tool for estimating masses of late-type stars. Recent advancements in techniques to measure [Fe/H] for M dwarfs, an increaseing number of benchmark sytems near the hydrogen burning limit, the availability of {\it Gaia} parallaxes, and the need for higher-precision masses of cool stars motivate an update to existing empirical mass-luminosity relations. Here we present an empirical relation between \mks\ and $M_*$ spanning 0.70$M_\odot$ down to the hydrogen burning limit ($\simeq0.075M_\odot$). The relation is calibrated from 62 nearby binary systems, whose orbits we determine using a a combination of our own monitoring with NIRC2 on Keck, archival adaptive optics data, and astrometry from the literature. Using these orbits and Kepler's law, we determine total masses of each system to better than 1\% in the best cases (median precision of 6.4\%). We join our mass determinations with resolved \mks\ magnitudes for binary component stars to derive an empirical \mmk\ relation, which yields masses of single stars to 2-3\% for $0.18M_\odot<M_*<0.70M_\odot$, and 3-6\% for $0.075M_\odot<M_*<0.18M_\odot$. Our relation is mostly consistent with earlier work, with differences attributable to issues with the input sample. We find [Fe/H] has a weak effect on the \mmk\ for the range of metallicities expected in the Solar neighborhood ($\simeq$1\% change in $M_*$), but may exceed uncertainties in the relation ($>3\%$) for subdwarfs. Our results also suggest that evolutionary models likely overestimate the role of [Fe/H] on the \mmk\ relation, but the difference is not significant with the current sample. Future studies can build on this sample to further explore the role of [Fe/H], as well as the effects that activity and youth have on the mass-luminosity relation.
\end{abstract}
...
%% make software on github work
%% More proofreading
%% cross-check names for all tables (might be some issues)
...
For all these areas, it is essential that we have a method to estimate the fundamental parameters of late-type dwarfs. Spectra, photometry, and distances of stars provide a relatively direct means to measure \teff\ \citep[e.g.,][]{Mann2013c}, luminosity \citep[e.g.,][]{2002AJ....124.2721R}, metallicity \citep[e.g.,][]{2005A&A...442..635B,2010ApJ...720L.113R}, and radii \citep[e.g., via Stefan-Boltzmann, ][]{Newton2015A,2018arXiv180404133K}. Masses, however, are much more difficult to infer from observational data alone. 
...
Basic parameters of the final 62 systems included in our analysis are given in Table~\ref{tab:sample}.

\section{Observations and Data Reduction}\label{sec:obs} 

\subsection{Adaptive Optics Imaging}\label{sec:ao}

We analyzed a mix of AO data from our own program with NIRC2/Keck, and images from the archives of Keck, the Canada France Hawaii Telescope (CFHT), the Very Large Telescope (VLT), and Gemini observatory. In general we analyzed all usable (e.g., non saturated, components resolved) regardless of observing mode and filter. 

For our analysis, we considered a single measurement epoch a set of images with a unique filter or target, or UT date. Each measurement consisted of a $\Delta m$ (in the given filter), separation, and position angle. 
...
We retrieved 41 epochs for 8 of our targets from the Gemini archive, all taken with the AO imager NIRI \citep{2003PASP..115.1388H} on Gemini North. All observations were taken between August 2008 and February 2011 and were taken with the assistance of the ALTtitude conjugate Adaptive optics for the InfraRed (ALTAIR). Most observations were taken with the f32 camera using broadband $J$-, $H$-, or $K$-band filters. All observations followed a 2-4 point dither pattern and took at least two images at each dither location. 
...
\subsection{Measuring astrometry and photometry}\label{sec:astrometry}
...
As part of the PSF fit, both methods yielded a measurement of the total flux of each star, as well as an estimate of the local background levels. We used this to measure $\Delta m$ in the relevant band. Data from all filters are used for astrometry, although only $\Delta K$ magnitude measurements were used in the estimate of \mks\ (Section~\ref{sec:mags}).
...
We adopted astrometric measurements (position angles and separations) from the literature for 51 of the 53 binaries analyzed here. To help identify sources of astrometry, we used the fourth catalog of interferometric observations of binary stars \citep[INT4,][]{Hartkopf:2001}. We only used measurements that included both a separation and position angle. In cases of literature data also available in one of the archives above, we adopted our own measurements over the literature data to ensure more homogenous measurements and errors. We did not utilize reported $\Delta$m from the literature, although in some cases this information was used to verify the astrometry was for the correct source. In total, we used 589 measurements (each including a separation and position angle) from 63 references. The literature astrometry was asymmetrically distributed; roughly half of the literature points were for just 9 targets. 
...
Two of the three targets missing a SpeX data are too warm to derive a metallicity from NIR spectra (Gl 792.1 and Gl 765.2), and the third (Gl 54) is was too far south to be observed with IRTF. 
...
Parallaxes for 54 of the targets were drawn from the literature. In general we adopted the most precise parallax available, although preference is given to those accounting for the binarity of the target and those with a baseline significantly different from the binary's orbital period. A few systems are in hierarchical triples with a wider companion that also has an independent parallax measurement, which we used where possible. For $\simeq$1/3 of the sample, parallaxes were drawn from the new reduction of the Hipparcos data \citep{van-Leeuwen:2007yq}. We drew parallaxes from \citet{Dupuy2017} along with their orbital solutions, and parallaxes from \citet{Benedict2016} for those overlapping with our own sample. Most of the remaining parallaxes were drawn from the general catalogue of trigonometric parallaxes \citep{van-Altena1995}.

Many of our targets do not have entries in the second data release of {\it Gaia} \citep[DR2, ][]{GaiaDr2,2018arXiv180409365G}. They were likely excluded because centroid shifts from orbital motion prevented a single-star solution (a requirement to be included in DR2). We also found significant differences between the {\it Gaia} values and earlier measurements \citep[including from {\it Gaia} DR1, ][]{gaiadr1}. Many wide triples in {\it Gaia} DR2 (where the third star is several arc seconds away, and hence easily resolved) have significantly different parallaxes reported for the unresolved pair than for the wider companion. Orbital motion is likely impacting the final parallax values or uncertainties. The problem should be fixed in future Gaia data releases when there is sufficient data to fit for binary parameters. We found no such issues with our other parallax sources. 

We ultimately used {\it Gaia} DR2 parallaxes for four systems, GJ1245AC, GJ277AC, GJ4287AB, and LP415-20AB, where we adopted the parallax of the wider common proper motion companion. In all cases the wider associated stars are not known to harbor another unresolved star, and hence they should not be impacted by the same binarity issue. GJ 570BC, Gl 695BC, GJ 2005BC, Gl 22AC, and 2M1047+40 also have nearby associated stars. However, Gl 570A is extremely bright ($G\simeq5$) and hence may have an unreliable parallax due to saturation in DR2, GJ 2005A has no entries in {\it Gaia} DR2, the available parallaxes (from {\it HST} and Hipparcos) for Gl 695A,BC and Gl 22AC,B are more precise than the ones for Gl 695A and Gl 22B from {\it Gaia} DR2, and the wide pair to 2M1047+40 is itself a tight binary \citep[LP 213-67AB, ][]{Dupuy2017}.
...
Our sample encompasses almost an order of magnitude in mass and hence a range of underlying stellar physics. No single $\alpha$ value is expected to fit over the full sequence. Instead, we assumed that $\alpha$ depends on \mks, which we approximated as a polynomial. This yields a \mmk\ relation of the form:
\begin{equation}\label{eqn:mmk}
\log \left( \frac{M_*}{M_\odot} \right) = \sum_{i=0}^{n} a_i(M_{K_S}-zp)^i,
\end{equation}
where $a_i$, are the $n$ fit coefficients. The constant $zp$ is a zero-point (or anchor) magnitude, which we fixed at 7.5. This approximately corresponded to the logarithmic central mass of stars in our sample. 
...
\startdata
 -0.657781 &  -0.202363 &  5.389494$\times10^{-3}$ &  5.499459$\times10^{-3}$ & -3.217666$\times10^{-4}$ & \nodata & 1.42 \\ 
 -0.642302 &  -0.198713 &  2.657887$\times10^{-7}$ &  4.761033$\times10^{-3}$ & \nodata & \nodata & 1.86 \\
\hline
 -0.650076 &  -0.203041 &  3.918380$\times10^{-3}$ &  5.385787$\times10^{-3}$ & -2.732850$\times10^{-4}$ &  0.036 & 1.54 \\
 -0.640213 &  -0.191602 & -1.510876$\times10^{-4}$ &  4.020492$\times10^{-3}$ & \nodata  &  0.033 & 1.78 \\
 \enddata
\label{tab:coeff}
...
\hline
 4.0 & 0.712 & 0.025 & 0.035 & K4.5 \\
 4.5 & 0.679 & 0.015 & 0.022 & K7.0 \\
 5.0 & 0.609 & 0.013 & 0.021 & M0.0 \\
 5.5 & 0.523 & 0.011 & 0.020 & M1.5 \\
 6.0 & 0.433 & 0.009 & 0.020 & M2.5 \\
 6.5 & 0.351 & 0.008 & 0.023 & M3.0 \\
 7.0 & 0.280 & 0.008 & 0.027 & M3.5 \\
 8.0 & 0.178 & 0.006 & 0.032 & M4.5 \\
 8.5 & 0.144 & 0.005 & 0.032 & M5.0 \\
 9.0 & 0.118 & 0.004 & 0.033 & M6.0 \\
 9.5 & 0.100 & 0.004 & 0.037 & M7.0 \\
10.0 & 0.086 & 0.004 & 0.046 & M8.0 \\
10.5 & 0.077 & 0.005 & 0.060 & M9.0 \\
11.0 & 0.073 & 0.006 & 0.080 & L1.0 \\
\hline
\multicolumn{5}{c}{[Fe/H] term, 4 $a_i$ terms}   \\
\hline
 4.0 & 0.731 & 0.028 & 0.038 & K4.5 \\
 4.5 & 0.679 & 0.015 & 0.023 & K7.0 \\
 5.0 & 0.603 & 0.011 & 0.018 & M0.0 \\
 5.5 & 0.518 & 0.010 & 0.020 & M1.5 \\
 6.0 & 0.432 & 0.009 & 0.021 & M2.5 \\
 6.5 & 0.353 & 0.007 & 0.021 & M3.0 \\
 7.0 & 0.284 & 0.006 & 0.021 & M3.5 \\
 8.0 & 0.182 & 0.004 & 0.024 & M4.5 \\
 8.5 & 0.146 & 0.004 & 0.028 & M5.0 \\
 9.0 & 0.120 & 0.004 & 0.032 & M6.0 \\
 9.5 & 0.100 & 0.004 & 0.037 & M6.5 \\
10.0 & 0.086 & 0.004 & 0.044 & M7.5 \\
10.5 & 0.076 & 0.004 & 0.053 & M9.0 \\
11.0 & 0.073 & 0.005 & 0.068 & L1.0 \\
\enddata
\label{tab:err}
...
To compare to the models, we fit the MIST grid points in the same manner as the empirical dataset following Equation~\ref{eqn:mmk3}. In Figure~\ref{fig:f} we show the posterior on $f$ from the model grid compared to that from the dynamical masses. While the posteriors have some overlap, more than 96\% of the difference between the two posteriors ($f_{\rm{model}}-f_{\rm{dynamical}}$) is greater than zero, indicating (with marginal significance) that the models overpredict the effect of [Fe/H] on the \mmk\ relation. If real, it is unclear if this difference between the models and empirical data applies to the full mass range considered; there is some indication that the metallicity effect is stronger for higher-mass stars in the sample. The models suggest $f$ is $\simeq$30\% larger for [Fe/H]$<-1.0$. Thus a set of subdwarf binaries will provide more leverage to test over what mass range, if any, the discrepancy with the models is statistically significant. 
...
We endeavored to improve on existing \mmk\ relations and explore the role of [Fe/H] by expanding the sample of calibrators and using new techniques to measure the metallicity of binary M dwarfs. As part of this effort, we have been monitoring a set of nearby late K and M dwarf visual binaries using adaptive optics imaging with the goal of mapping their orbits and estimating their masses via Kepler's Laws. We combined these data with similar data from Keck, CFHT, Gemini, and VLT archives, as well as astrometric measurements from the literature. The literature astrometry was particularly important to provide the baseline on long-period systems. We used these data and parallaxes from the literature or derived from MEarth astrometry to measure the total masses of a set of 53 binaries. We combined this with nine ultracool binaries from \citep{Dupuy2017}, which were analyzed in a nearly identical way to the procedure used in this paper and provide significantly improved coverage near the hydrogen burning limit. 

We used our dynamical masses and resolved \mks\ magnitudes to fit for an empirical relation between $M_*$ and \mks. The result can provide masses precise to 2-3\% from $0.3-0.6M_\odot$ and $<5$\% over the whole M dwarf sequence. The relation also covers almost an order of magnitude in $M_*$, from $\simeq0.7M\odot$ down to the hydrogen burning limit ($\simeq0.075M_\odot$). The scatter around the best-fit relation is larger than expected from the mass uncertainties, especially when considering that errors in \mks\ and $M_*$ are correlated. This might be due to underestimated errors in the parallaxes (from centroid motion of the binary), or intrinsic scatter from additional physics (e.g., metallicity and age). 
...
We repeated this process with 5000 unique combinations of the 183 stars for 12 different filter/instrument combinations. Forr each filter and instrument combination we computed a best-fit line to $\Delta K_S -\Delta K_X$ as a function of $\Delta K_X$. We show four examples in Figure~\ref{fig:mags}. For the majority of the filters considered, the trend is insignificant compared to errors in the underlying spectra and absolute calibration ($\simeq0.02$\,mag). We did not apply a correction in these cases.   
