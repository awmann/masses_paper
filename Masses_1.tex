...
%% To do:
%% compare mass ratios
%% forward modeling metallicity
%% fit with an extra error term
%% change rchi^2 to chi^2 and dof
%% a"^3/P^2 in the table
%% Applications section (Trappist-1, etc.)
%% general improvement of plots in appendix
...
In total we measured or gathered 1100 unique separation and position angle measurements, approximately half of which were from the literature. The number of individual measurements makes it appear astrometry from our monitoring program with Keck/NIRC2 was a minor component of the orbits (154 datasets). However, the NIRC2 points are typically 1-2 orders of magnitude more precise than those from the literature. If we instead compared the total number of measurements weighted by their uncertainties, the NIRC2 data contributed $\gtrsim10\times$ more total astrometric information than the literature data. Measurements from CFHT/PUEO had a comparable total contribution to the literature data, which had $\simeq2\times$ the effect of all NaCo measurements and $\simeq3\times$ that of data from NIRI. This is only a rough proxy for the relative role of each data source, since literature data also tended to be heavily concentrated on the best-characterized systems, and the usefulness of a given measurement also depends on where in the orbit it was taken. 
...
\caption{Example results of our orbit fitting procedure for Gl301AB (left), Gl469AB (middle), and GJ3454AB (right). Red points are individual measurements of the separation and position angle. Black solid line shows the best-fit (highest likelihood). Grey lines are drawn by randomly sampling 100 orbit fits from the MCMC chain to display an estimate of the errors. The dotted line connects periastron passage, with an arrow pointing in the orbital direction, and the dashed-line indicates the line of nodes. For Gl301AB, there are no data for a small section orbit, but the fit was relatively well constrained. For Gl469AB, high-quality astrometry is available for the full orbit, and the resulting errors on orbital parameters are so small that the grey lines cannot be seen. All other orbits are shown in Appendix~\ref{sec:a1}.}
\label{fig:orbits}
\end{center}
\end{figure*}
