...
We analyzed a mix of AO data from our own program with NIRC2 on Keck with images from the archives of Keck, the Canada France Hawaii Telescope (CFHT), the Very Large Telescope, and Gemini observatory. In general we analyzed all usable (e.g., non saturated, components resolved) regardless of observing mode and filter. For our analysis, we considered a single epoch all data taken for one target and filter within a single night. 

We separate the observations and reduction by instrument/telescope below. The full list of astrometry and contrast measurements is given in Table~\ref{tab:astrom}. 
...
We retrieved 14 epochs for 7 of our targets from the Gemini archive, all taken with the AO imager NIRI. All observations were taken between August 2008 and February 2011. Most observations were taken with the f32 camera, but using wide range of filters. All observations followed a 2-4 point dither pattern and took at least two images at each dither location. 
